...
private: void markDirty(); void setError(const QString &message); void startTranslation(const QVariantList &segments, const QString &activeSegmentId = QString()); void applyPatches(const QVariantList &patches); void autosave(); QString historyPath() const; void loadHistory(); void addHistory();
    ModelManager *m_models = nullptr; RuntimeManager *m_runtimes = nullptr; TranslationModelSession *m_session = nullptr; TranslationProject m_project; QFutureWatcher<QVariantList> m_watcher; QTimer m_autosave; std::shared_ptr<std::atomic_bool> m_cancelToken; bool m_dirty = false; bool m_processing = false; QString m_activeSegmentId; int m_progress = 0; QString m_error; QVariantList m_history;
};
} // namespace LAStudio
